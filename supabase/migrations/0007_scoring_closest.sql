-- =============================================================================
-- 0007_scoring_closest.sql — Spieltipp-Punkte mit "am nächsten dran"
--
--   Exaktes Ergebnis                         → 7
--   Am nächsten dran (kleinste Tor-Distanz unter den nicht-exakten
--     Tipps eines Spiels; bei Gleichstand alle)               → 5
--   Richtige Tendenz (Sieg/Unentschieden/Niederlage)          → 3
--   Falsch                                                    → 0
--
-- Sonderwetten unverändert. avatar_url (0004) beibehalten.
-- =============================================================================
...
AS
WITH
-- Pro Tipp: exakt?, Tor-Distanz, Tendenz korrekt?
match_scored AS (
  SELECT
    mt.user_id,
    mt.match_id,
...
-- Kleinste Distanz je Spiel (nur nicht-exakte Tipps zählen für "am nächsten dran")
match_mindist AS (
  ...
  SELECT
    ...
    SUM(
      CASE
        ...
        WHEN ms.dist = md.mindist            THEN 5
